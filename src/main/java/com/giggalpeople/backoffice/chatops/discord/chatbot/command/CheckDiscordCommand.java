...
public class CheckDiscordCommand {
    ...
    public static Map<String, List<String>> manufactureCommand (MessageReceivedEvent event, String userMessage) {
        ...
        if (crewRole.equals("")) {
            userCommandMap.put("권한 없음", Collections.singletonList(event.getAuthor().getAsMention() + "님은 해당 명령어를 사용할 권한이 없습니다."));
        ...
        if (inDiscordCheckCrewGrade(crewRole, userCommandArray)) {
            for (int index = 0; index < userCommandArray.length; index++) {
                ...
                if (CommandUtil.commandCheck(userCommandArray[index])) {
                ...
                } else if (userCommandArray[index].matches("명령어") || userCommandArray[index].matches("안녕") || userCommandArray[index].matches("소개") || userCommandArray[index].matches("누구야")) {
                    ...
                    try {
                        ...
                        throw new DiscordBotException(API_RESPONSE_NOT_FOUND, API_RESPONSE_NOT_FOUND.getMessage(String.valueOf(ioException)));
                    }
            ...
            DiscordBotResponseMessageUtil.sendMessage(event, generalMessage, DiscordBotResponseMessageUtil.createCommonEmbed(generalMessage.length() + embedMessage.length(), embedBuilder));
            return userCommandMap;
        }
    }
