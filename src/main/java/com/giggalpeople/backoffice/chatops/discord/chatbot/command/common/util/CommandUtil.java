...
public class CommandUtil {
    ...
    public static Map<String, List<String>> movementFromCommand(MessageReceivedEvent event, Map<String, List<String>> userCommandMap) throws IOException {
        ...
        for (Map.Entry<String, List<String>> entry : userCommandMap.entrySet()) {
            ...
            switch (key) {
                case "크루목록조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 크루 전체 정보는 아래와 같아요!", BackOfficeAPICaller.callPeopleManagementListSearchApi(event, new URL(HttpUtil.crewSearchCreateURL(event, userCommandMap))));
                    break;
                case "크루상세조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 해당 크루 상세 정보는 아래와 같아요!", Collections.singletonList(BackOfficeAPICaller.crewDetailAPICall(event, new URL(HttpUtil.crewDetailCreateURL(event, value)))));
                    break;
                case "지원자목록조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 지원자 전체 정보는 아래와 같아요!", BackOfficeAPICaller.callPeopleManagementListSearchApi(event, new URL(HttpUtil.suggestSearchCreateURL(event, userCommandMap))));
                    break;
                case "지원자상세조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 해당 지원자 상세 정보는 아래와 같아요!", Collections.singletonList(BackOfficeAPICaller.suggestDetailAPICall(event, new URL(HttpUtil.suggestDetailCreateURL(event, value)))));
                    break;
                case "이용자목록조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 서비스 Application 이용자 접속 및 요청 정보 목록 조회 결과는 아래와 같아요!", BackOfficeAPICaller.callUserInfoListSearchApi(event, new URL(HttpUtil.userInfoSearchCreateURL(event, userCommandMap))));
                    break;
                case "이용자상세조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 서비스 Application 이용자 접속 및 요청 정보는 아래와 같아요!", Collections.singletonList(BackOfficeAPICaller.userInfoDetailAPICall(event, new URL(HttpUtil.userInfoDetailCreateURL(event, value)))));
                    break;
                case "로그목록조회":
                    resultMap.put(event.getAuthor().getName() + "님 서버 에러 목록 조회 결과는 아래와 같아요!", BackOfficeAPICaller.callErrorLogListSearchApi(event, new URL(HttpUtil.errorLogSearchCreateURL(event, userCommandMap))));
                    break;
                case "로그상세조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 해당 Error Log 정보는 아래와 같아요!", Collections.singletonList(BackOfficeAPICaller.errorLogDetailAPICall(event, new URL(HttpUtil.errorLogDetailCreateURL(event, value)))));
                    break;
                case "서버자원조회":
                    resultMap.put(event.getAuthor().getName() + "님 기깔나는 사람들 해당 WAS 자원 정보는 아래와 같아요!", Collections.singletonList(DiscordBotResponseMessageUtil.getServerResourceInfo(event, value)));
                    break;
                case "명령어":
                    resultMap.put(event.getAuthor().getName() + "님 저에게 명령을 하시려면 이렇게 말씀해 주셔야 해요!", Collections.singletonList(CommandUtil.commandManual(event)));
                    break;
                default:
                    resultMap.put(event.getAuthor().getName() + DiscordBotResponseMessage.existCommandRandomMessage("안녕"), Collections.singletonList(CommandUtil.checkBodyExistCommand(event, key)));
                    break;
            }
        }
    ...
    public static void insertInCommandOptionMap(List<String> userCommandOptionKeyValue, Map<String, String> userCommand) {
        String mapKey = "";
        String mapValue = "";
    ...
    public static void checkUserCommandForSearchErrorLogPagination(StringBuilder url, String key, String value) {
        if (key.contains("현재페이지")) {
            ...
            url.delete(0, url.length());
            url.append(Environment.initPrefixAPIURL()).append(API_CALLER_DISCORD_BOT + LOG + "/lists?page=").append(value);
    ...
    public static void checkUserCommandForSearchUserInfoPagination(StringBuilder url, String key, String value) {
        if (key.contains("현재페이지")) {
            ...
            if (checkUrl.contains(Environment.initPrefixAPIURL() + API_CALLER_DISCORD_BOT + CONNECTED_USER + "/lists?page=")) {
                url.append("&perPageNum=").append(value);
    ...
    public static boolean processorUrnUserCommandForDate(MessageReceivedEvent event, StringBuilder url, String key, String value) {
        if (DataTypeChangerUtil.checkDateRangeTypeForSearchWord(value)) {
            ...
            String errorEmbedMessage = event.getAuthor().getAsMention() + "님 날짜 검색을 명령 하셨으나, 날짜 범위 값이 잘 못 입력 되었어요! 날짜 검색 방법 : yyyy-MM-dd~yyyy-MM-dd";
