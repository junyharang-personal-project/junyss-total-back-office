...
public class BackOfficeAPICaller {
    ...
    public static String userInfoDetailAPICall(MessageReceivedEvent event, URL url) throws IOException {
        CrewGrade crewGrade = CheckDiscordCommand.checkCrewGradeString(event.getMessage().getMember().getRoles().get(0).getName());
    ...
    private static List<String> createPeopleManagementResponseMessage(JSONObject jsonObject, URL url, StringBuilder resultMessage) {
        ...
        for (int index = 0; index < data.length(); index ++) {
            ...
            if (HttpUtil.discordBotSearchAPIURL(url)) {    /* 크루 전체 목록 조회 시 */
                resultMessage.append(DiscordBotResponseMessageUtil.createFindByCrewList(indexOfSuggest, dataJSONObject));
                checkLastIndex(index, data, resultMessage, pagination);

            } else {                                        /* 지원자 전체 목록 조회 시 */
                resultMessage.append(DiscordBotResponseMessageUtil.createFindBySuggestList(indexOfSuggest, dataJSONObject));
                checkLastIndex(index, data, resultMessage, pagination);
            }
    ...
    private static List<String> createUserInfoResponseMessage(JSONObject jsonObject, StringBuilder resultMessage) {
        ...
        for (int index = 0; index < data.length(); index ++) {
            ...
            resultMessage.append(DiscordBotResponseMessageUtil.createFindByUserInfoList(connectedUserRequestInfoID, dataJSONObject));
            checkLastIndex(index, data, resultMessage, pagination);
            resultList.add(resultMessage.toString());
            resultMessage.delete(0, resultMessage.length());
        }
    ...
    public static String suggestDetailAPICall(MessageReceivedEvent event, URL url) throws IOException {
        ...
        return DiscordBotResponseMessageUtil.createDetailSuggestSearchResponseMessage(Objects.requireNonNull(APICallUtil.callDiscordBotGetAPI(url)).getJSONObject("data"));
    }
    ...
    public static String crewDetailAPICall(MessageReceivedEvent event, URL url) throws IOException {
        ...
        return DiscordBotResponseMessageUtil.createDetailCrewSearchResponseMessage(Objects.requireNonNull(APICallUtil.callDiscordBotGetAPI(url)).getJSONObject("data"));
    }
    ...
    public static String errorLogDetailAPICall(MessageReceivedEvent event, URL url) throws IOException {
        ...
        JSONObject apiResponseData = Objects.requireNonNull(APICallUtil.callDiscordBotGetAPI(url)).getJSONObject("data");

        if (crewGrade.getGradeNum() > 3) {
            return DiscordBotResponseMessageUtil.forGeneralCrewCreateDetailErrorLogSearchResponseMessage(apiResponseData);
        } else {
