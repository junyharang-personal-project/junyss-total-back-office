...
public class GiggalDiscordListener extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		Map<String, List<String>> resultMap = CheckDiscordCommand.manufactureCommand(event,
		...
		for (Map.Entry<String, List<String>> apiReturnMessage : resultMap.entrySet()) {
			if (apiReturnMessage.getKey().contains("내용 없음") || apiReturnMessage.getValue()
				.contains(user.getName() + "님 명령어를 확인해 주세요.")) {
