...
public class HttpUtil {
        ...
        try {
            return checkSearchSubject(event,
                    Environment.initPrefixAPIURL()
                            .append(API_CALLER_DISCORD_BOT + CONNECTED_USER + "/lists?page=" + 0 + "&perPageNum=" + 0 + "&displayPageNum=" + 0),
        ...
        for (String userCommandOption : userCommandOptionList) {
            if (DataTypeChangerUtil.checkRegularPK(userCommandOption)) {
                url.append(API_CALLER_DISCORD_BOT + CONNECTED_USER + "/details")
                        ...
                        .append("&crewGrade=")
                        .append(CheckDiscordCommand.checkCrewGradeString(event.getMessage().getMember().getRoles().get(0).getName()));
    ...
    public static String errorLogSearchCreateURL(MessageReceivedEvent event, Map<String, List<String>> userCommandMap) {
        try {
            return checkSearchSubject(event,
                    Environment.initPrefixAPIURL()
                            .append(API_CALLER_DISCORD_BOT + LOG + "/lists?page=" + 0 + "&perPageNum=" + 0 + "&displayPageNum=" + 0),
     ...
     * @return API 호출을 위한 URL
     */
    private static String checkSearchSubject(MessageReceivedEvent event, StringBuilder url, List<String> searchCommandOption, int check) throws IOException {
    ...
    private static boolean createSuggestSearchURL(MessageReceivedEvent event, StringBuilder url, List<String> suggestSearchCommandOption) {
        ...
        for (Map.Entry<String, String> entry : userCommand.entrySet()) {
            ...
            if (key.contains(SUGGEST_DATE.getDescription()) || key.contains(BIRTH_DATE.getDescription()) || key.contains(MEET_DATE.getDescription())) {
    ...
    private static boolean createCrewSearchURL(MessageReceivedEvent event, StringBuilder url, List<String> suggestSearchCommandOption) {
        List<String> userCommandOptionKeyValue;
        Map<String, String> userCommand = new HashMap<>();
        boolean processCheck = true;
    ...
    private static boolean createErrorLogSearchURL(MessageReceivedEvent event, StringBuilder url, List<String> errorLogSearchCommandOption) {
        List<String> userCommandOptionKeyValue;
        Map<String, String> userCommand = new HashMap<>();
        boolean processCheck = true;
     ...
     * @return URL이 정상적으로 만들어지면 True 아니면 False
     */
    private static boolean createUserInfoSearchURL(MessageReceivedEvent event, StringBuilder url, List<String> searchCommandOption) {
        List<String> userCommandOptionKeyValue;
        Map<String, String> userCommand = new HashMap<>();
        boolean processCheck = true;
...
//    }
    ...
    public static boolean checkParticipation(String value, StringBuilder url) {
        if (Participation.CHECK.getSaveParticipationInfo().equals(value)) {
            ...
            url.append("&searchWord=");
            url.append(HttpUtil.createCommonEncodeURL(Participation.TEAM_LEADER_JOIN_REFUSAL.getSaveParticipationInfo()));

            return true;

        } else if (Participation.PROJECT_LEADER_JOIN_REFUSAL.getSaveParticipationInfo().equals(value)) {
            url.append("&searchWord=");
            url.append(HttpUtil.createCommonEncodeURL(Participation.PROJECT_LEADER_JOIN_REFUSAL.getSaveParticipationInfo()));
