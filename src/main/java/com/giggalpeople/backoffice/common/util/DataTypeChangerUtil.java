...
public class DataTypeChangerUtil {
    ...
    public static String changeSearchWordForPart (String searchWord) {
        ...
        if (searchWord.toLowerCase().contains("project") || searchWord.toLowerCase().contains("manager") || searchWord.toLowerCase().contains("pm")) {
            return "Project Manager(PM)";
        } else if (searchWord.contains("서비스") || searchWord.contains("기획") || searchWord.toLowerCase().contains("plan") || searchWord.toLowerCase().contains("service")) {
            return "서비스 기획";
        } else if (searchWord.toLowerCase().contains("web") || searchWord.toLowerCase().contains("app") || searchWord.toLowerCase().contains("design") || searchWord.toLowerCase().contains("ui") || searchWord.toLowerCase().contains("ux") || searchWord.contains("디자")) {
            return "WEB / APP 디자이너";
        } else if (searchWord.toLowerCase().contains("back") || searchWord.contains("백엔") || searchWord.contains("빽엔") || searchWord.contains("빼갠") || searchWord.contains("빼겐") || searchWord.contains("서버") || searchWord.toLowerCase().contains("server")) {
            return "BackEnd 개발자";
        } else if (searchWord.toLowerCase().contains("front") || searchWord.contains("프엔") || searchWord.toLowerCase().contains("프론트")) {
            return "FrontEnd 개발자";
        } else if (searchWord.toLowerCase().contains("dmso") || searchWord.contains("dev") || searchWord.toLowerCase().contains("ops") || searchWord.toLowerCase().contains("ml") || searchWord.toLowerCase().contains("sec") || searchWord.toLowerCase().contains("infra") || searchWord.toLowerCase().contains("인프라") || searchWord.toLowerCase().contains("데브") || searchWord.toLowerCase().contains("옵브") || searchWord.toLowerCase().contains("엠엘") || searchWord.toLowerCase().contains("쎅") || searchWord.toLowerCase().contains("보안")) {
            return "DMSO(DevMlSecOps)";
        } else if (searchWord.toLowerCase().contains("hat") || searchWord.toLowerCase().contains("hacker")) {
            return "화이트 해커";
        } else if (searchWord.toLowerCase().contains("swi") || searchWord.toLowerCase().contains("ios") || searchWord.toLowerCase().contains("apple") || searchWord.contains("아이오에스") || searchWord.toLowerCase().contains("스위프트") || searchWord.toLowerCase().contains("스프")) {
            return "IOS 개발자";
        } else if (searchWord.toLowerCase().contains("an") || searchWord.toLowerCase().contains("android") || searchWord.toLowerCase().contains("google") || searchWord.contains("안드") || searchWord.toLowerCase().contains("로이드") || searchWord.toLowerCase().contains("구글")) {
            return "Android 개발자";
        } else if (searchWord.toLowerCase().contains("ai") || searchWord.contains("인공") || searchWord.contains("지능") || searchWord.contains("머신") || searchWord.contains("러닝")) {
            return "AI 개발자";
        } else {
            return searchWord;
        }
    ...
    public static void checkAgreeType(CrewSuggestPeopleManagementSearchDTO crewSuggestSearchDTO) {
        if (crewSuggestSearchDTO.getSearchType().equals("PRIVACY_INFO_AGREE") && crewSuggestSearchDTO.getSearchWord().equals("동의")) {
            crewSuggestSearchDTO.setInDBPrivacyInfoAgree(true);
        } else if (crewSuggestSearchDTO.getSearchType().equals("PRIVACY_INFO_AGREE") && crewSuggestSearchDTO.getSearchWord().equals("미동의")){
            crewSuggestSearchDTO.setInDBPrivacyInfoAgree(false);
        } else if (crewSuggestSearchDTO.getSearchType().equals("ADDENDUM_INFO_AGREE") && crewSuggestSearchDTO.getSearchWord().equals("동의")) {
            crewSuggestSearchDTO.setInDBAddendumInfoAgree(true);
        } else if (crewSuggestSearchDTO.getSearchType().equals("ADDENDUM_INFO_AGREE") && !crewSuggestSearchDTO.getSearchWord().equals("미동의")) {
