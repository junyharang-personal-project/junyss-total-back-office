...
public class UserInfoDetailResponseDTO {
    ...
    public UserInfoDetailResponseDTO toDTO(ConnectedUserInfoVO connectedUserInfoVO) {
        ...
        userInfoDetailResponseDTO.dataCreatedDateTime = connectedUserInfoVO.getDataCreatedDate() + " " + connectedUserInfoVO.getDataCreatedTime();
        ...
        try {
            userInfoDetailResponseDTO.userIP = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getUserIP());
            userInfoDetailResponseDTO.userEnvironment = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getUserEnvironment());
            userInfoDetailResponseDTO.userLocation = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getUserLocation());
            userInfoDetailResponseDTO.requestHeader = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getRequestHeader());
            userInfoDetailResponseDTO.userCookies = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getUserCookies());
            userInfoDetailResponseDTO.requestParameter = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getRequestParameter());
            userInfoDetailResponseDTO.requestBody = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(connectedUserInfoVO.getRequestBody());

        } catch (JsonProcessingException jsonProcessingException) {
            throw new ConnectedUserException(NOT_EXIST_CONNECTED_USER_JSON_PARSING_ERROR, NOT_EXIST_CONNECTED_USER_JSON_PARSING_ERROR.getMessage(String.valueOf(jsonProcessingException)));
        }
