...
/**
 * <h2><b>이용자 접속 및 요청 정보 검색을 위한 검색 DTO</b></h2>
 */

@Schema(description = "이용자 접속 및 요청 정보 검색을 위한 검색 DTO")
@Data
@NoArgsConstructor
@AllArgsConstructor
public class UserInfoSearchDto {

	@Schema(description = "검색 조건(종류) - CONNECTED_USER_REQUEST_ID: 이용자 접속 ID(순번), " +
		"USER_CONNECTED_DATE: 이용자 서비스 접속일, " +
		"SERVER_NAME: Server Name, " +
		"SERVER_IP: Server IP, " +
		"USER_IP: 이용자 IP 주소, ", nullable = true, example = "SERVER_NAME")
	private UserInfoSearchType inputSearchType;
	...
	@Schema(description = "검색어 - 검색 종류가 내부 서버일 경우 내부 서버 종류로 검색 가능 : " +
		"1. 통합 관리 서버 = 통합관리서버, ", nullable = true, example = "192.168.0.3")
	...
	@Schema(description = "날짜 검색 하나를 위한 날짜", nullable = true, example = "2023-03-01")
	@Pattern(regexp = ".*(\\d{4})-(\\d{2})-(\\d{2})")
	private String date;
	...
	public String getSearchType() {
		...
		if (this.inputSearchType != null && searchWord != null) {
			String serverNameSearchWord = StringUtil.checkSearchCommandForInternalServerName(
				this.inputSearchType.getDescription(), this.searchWord);

			if (!serverNameSearchWord.equals("")) {
				setSearchWord(serverNameSearchWord);
			}

			return putSearchTypeValue(this.inputSearchType);
		} else if (this.searchWord == null && this.inputSearchType != null) {
			return putSearchTypeValue(this.inputSearchType);
		}
		this.searchType = "";
		return "";
	 ...
	 */
	private String putSearchTypeValue(UserInfoSearchType inputSearchType) {
