...
nginxDockerContainerRun() {
  ...
  do
    ...
    then
      ...
      echo "[$NOW] [INFO] 최초 Nginx Blue Container 기동 할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
      ...
      echo "[$NOW] [INFO] Nginx Green Container 기동 할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
...
checkNginxStatus() {
  ...
  echo "[$NOW] [INFO] ${nginxContainerAndHostName} 기동 여부를 확인할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  ...
  echo "[$NOW] [INFO] ${nginxContainerAndHostName} 기동 여부 확인 대상 Container ID : ${containerId} " >> $LOG_DIR/"$NOW"-deploy.log 2>&1

  if [[ $checkContainerStatus != "Up"* ]];
  then
    echo "[$NOW] [INFO] ${nginxContainerAndHostName} Container 기동 중이지 않아 기동 합니다."
    echo "[$NOW] [INFO] ${nginxContainerAndHostName} Container 기동 중이지 않아 기동 합니다." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    ...
    echo "[$NOW] [INFO] ${nginxContainerAndHostName} Container 기동 중이에요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
