...
checkNginxStatus() {
  echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} 기동 여부를 확인할게요."
  echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} 기동 여부를 확인할게요." >> $LOG_DIR/"$SAVE_LOG_DATE"-deploy.log 2>&1
  ...
  echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} 기동 여부 확인 대상 Container ID : ${containerId} "
  echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} 기동 여부 확인 대상 Container ID : ${containerId} " >> $LOG_DIR/"$SAVE_LOG_DATE"-deploy.log 2>&1

  if [[ $checkContainerStatus != "Up"* ]];
  then
    echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} Container 기동 중이지 않아 기동 합니다."
    echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} Container 기동 중이지 않아 기동 합니다." >> $LOG_DIR/"$SAVE_LOG_DATE"-deploy.log 2>&1
    ...
    echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} Container 기동 중이에요."
    echo "[$NOW] [INFO] ${NGINX_CONTAINER_NAME} Container 기동 중이에요." >> $LOG_DIR/"$SAVE_LOG_DATE"-deploy.log 2>&1
