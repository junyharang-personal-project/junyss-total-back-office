...
checkLogDirectory() {
  ...
  then
    ...
    echo "[INFO] 기깔나는 사람들 통합 관리 서버 API 무중단 배포 서버 작업 중 Application Docker Container Health Check 작업이 시작 되었어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    echo "======================================[$NOW] 통합 백 오피스 api Application Docker Container Health Check======================================" >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    ...
    echo "[$NOW] [INFO] Nginx Health Check 대상 Container Name: ${TARGET_CONTAINER_NAME}" >> $LOG_DIR/"$NOW"-deploy.log 2>&1
...
nginxHealthCheck() {
  ...
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} Health Check 시작할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  ...
  echo "[$NOW] [INFO] 사용 명령어: curl -I http://${SERVER_IP}:${nginxExternalPortNumber}/api/test/profile | grep -oP 'HTTP/1.1 \K\d+' " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  ...
  do
    ...
    then
      ...
      echo "[$NOW] [WARN] ${TARGET_CONTAINER_NAME}가 기동 중이지만, 내부 서비스에 문제가 있어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
      echo "[$NOW] [WARN] ${TARGET_CONTAINER_NAME} 삭제 및 재 기동 실시합니다."
      echo "[$NOW] [WARN] ${TARGET_CONTAINER_NAME} 삭제 및 재 기동 실시합니다." >> $LOG_DIR/"$NOW"-deploy.log 2>&1

      nginxDockerContainerErrorRemove "${TARGET_CONTAINER_NAME}" "${nginxExternalPortNumber}"
    fi
    ...
    then
      ...
      echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} Container 상태가 정상이에요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
      ...
      echo "[$NOW] [ERROR] ${TARGET_CONTAINER_NAME} Container 상태에 문제가 있어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
      ...
      echo "[$NOW] [ERROR] ${TARGET_CONTAINER_NAME} Container 종료 및 삭제 시도할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    ...
    if [ "${retryCount}" -eq 10 ];
      then
        ...
        echo "[$NOW] [ERROR] ${TARGET_CONTAINER_NAME} Container Health 상태 문제가 있어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    ...
    echo "[$NOW] [WARN] ${TARGET_CONTAINER_NAME} Health Check 작업에 실패하였어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
...
nginxDockerContainerErrorRemove() {
  ...
  echo "[$NOW] [INFO] 기존 ${TARGET_CONTAINER_NAME} Docker Container 중지 및 삭제 작업을 시작할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  ...
  echo "[$NOW] [INFO] 컨테이너 이름 및 Host Name : ${TARGET_CONTAINER_NAME}  : " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 Port Number : ${portNumber} "
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 Port Number : ${portNumber} : " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 Docker Image Name : ${APPLICATION_DOCKER_IMAGE_NAME} "
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 Docker Image Name : ${APPLICATION_DOCKER_IMAGE_NAME} " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  echo "[$NOW] [INFO] 종료 및 제거할 기존 컨테이너 이름 : ${TARGET_CONTAINER_NAME} "
  echo "[$NOW] [INFO] 종료 및 제거할 기존 컨테이너 이름 : ${TARGET_CONTAINER_NAME} " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 ID : ${stopContainerId} "
  echo "[$NOW] [INFO] ${TARGET_CONTAINER_NAME} 컨테이너 ID : ${stopContainerId} " >> $LOG_DIR/"$NOW"-deploy.log 2>&1
  ...
  then
    ...
    echo "[$NOW] [ERROR] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 종료 작업 실패 하였어요. 스크립트 종료 합니다." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    exit 1

  else
    echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 종료 작업 성공 하였어요."
    echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 종료 작업 성공 하였어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 삭제 작업 진행할게요."
    echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 삭제 작업 진행할게요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
    ...
    then
      ...
      echo "[$NOW] [ERROR] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 삭제 작업 실패 하였어요. 스크립트 종료 합니다." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
      exit 1

    else
      echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 삭제 작업 성공 하였어요."
      echo "[$NOW] [INFO] 기존 동작 중이던 Container 이름: ${TARGET_CONTAINER_NAME}, Container ID: ${stopContainerId} 삭제 작업 성공 하였어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
...
echo "[$NOW] [INFO] 기깔나는 사람들 통합 관리 서버 API 무중단 배포 작업 중 Nginx Docker Container Health Check 작업이 끝났어요." >> $LOG_DIR/"$NOW"-deploy.log 2>&1
